...
export class HomeComponent implements OnInit, OnDestroy {
  imageUrl: string = 'https://localhost:7063/UploadMissionImage/Mission/';
  missionList: any[] = [];
  userList: any[] = [];
  page = 1;
  missionPerPages = 9;
    ...
    if (data.result == 1) {
      this.missionList = data.data.map((x: any) => {
        ...
        return {
          ...
          missionSkillId: x.missionSkillId,
          // missionImages: missionimg.split(',', 1)
          missionImages: missionimg.split(',')[0],
          missionThemeName: x.missionThemeName,
          missionSkillName: x.missionSkillName,
          missionStatus: x.missionStatus,

          // ✅ Added logic:
          missionApplyStatus: x.isApplied ? 'Applied' : 'Apply',
          missionApproveStatus: x.applicationStatus === 'Approved' ? 'Approved' : '',
      ...
      this.totalMission = this.missionList.length;
    } else {
      this._toast.error({
        detail: 'ERROR',
        summary: data.message,
  ...
  if (tokenDetail == null || tokenDetail.userType != 'user') {
    ...
    // ✅ FIX: Set appliedDate in ISO format for .NET compatibility
    // this.appliedDate = moment().toISOString();
    // this.appliedDate = new Date().toISOString(); // ✅ Native JS solution
    this.appliedDate = moment().utc().format(); // or: moment().toISOString()
