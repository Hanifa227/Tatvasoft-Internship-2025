...
export class MissionApplicationComponent implements OnInit, OnDestroy {
  ...
  approveMissionApplication(value: any) {
  const payload = value.id; // just the id (number)

  this._service.missionApplicationApprove(payload).subscribe(
    (data: any) => {
      if (data.result === 1) {
        this._toast.success({ detail: "SUCCESS", summary: data.data, duration: APP_CONFIG.toastDuration });
  ...
  deleteMissionApplication(value: any) {
  const missionApplicationDeleteSubscription = this._service.missionApplicationDelete(value.id).subscribe(
    (data: any) => {
      if (data.result == 1) {
        this._toast.success({ detail: "SUCCESS", summary: data.data, duration: APP_CONFIG.toastDuration });
        setTimeout(() => {
