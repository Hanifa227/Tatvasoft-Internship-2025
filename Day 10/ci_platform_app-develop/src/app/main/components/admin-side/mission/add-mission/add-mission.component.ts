...
export class AddMissionComponent implements OnInit, OnDestroy {
  ...
  formData.append('MissionTitle', value.missionTitle);
  formData.append('MissionDescription', value.missionDescription);
  formData.append('MissionOrganisationName', 'Demo Org');
  formData.append('MissionOrganisationDetail', 'Some details');
  formData.append('CountryId', value.countryId);
  formData.append('CityId', value.cityId);
  formData.append('StartDate', new Date(value.startDate).toISOString());
  formData.append('EndDate', new Date(value.endDate).toISOString());
  formData.append('MissionType', 'time'); // or goal, based on dropdown
  formData.append('TotalSheets', value.totalSheets);
  formData.append('MissionThemeId', value.missionThemeId);
  formData.append('MissionSkillId', value.missionSkillId);
  formData.append('MissionImages', imageUrl.map((e) => e.replace(/\s/g, '')).join(','));

  // ✅ Step 3: Call API
  this._service.addMission(formData).subscribe({
    next: (res: any) => {
      if (res.result === 1) {
        this._toast.success({
          detail: 'SUCCESS',
