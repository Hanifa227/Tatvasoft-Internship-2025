...
export class UpdateMissionComponent implements OnInit, OnDestroy {
  ...
  getMissionSkillList() {
    const sub = this._service.getMissionSkillList().subscribe((data: any) => {
      if (data.result == 1) {
        this.missionSkillList = data.data
      } else {
        this._toast.error({ detail: "ERROR", summary: data.message, duration: APP_CONFIG.toastDuration })
  ...
  getMissionThemeList() {
    const sub = this._service.getMissionThemeList().subscribe((data: any) => {
      if (data.result == 1) {
        this.missionThemeList = data.data
      } else {
        this._toast.error({ detail: "ERROR", summary: data.message, duration: APP_CONFIG.toastDuration })
  ...
  fetchDetail(id: any) {
    const sub = this._service.missionDetailById( id).subscribe((data: any) => {
      if (data.result !== 1) {
        this._toast.error({ detail: "ERROR", summary: data.message || "Failed to fetch mission details", duration: APP_CONFIG.toastDuration });
        return;
      }
      ...
      const patchObj = {
        ...
        missionTitle: this.editData.missionTitle || "",
        missionDescription: this.editData.missionDescription || "",
        missionOrganisationName: this.editData.missionOrganisationName || "",
        missionOrganisationDetail: this.editData.missionOrganisationDetail || "",
        missionType: this.editData.missionType || "",
        countryId: this.editData.countryId || "",
        cityId: this.editData.cityId || "",
        startDate: this._datePipe.transform(this.editData.startDate, "yyyy-MM-dd"),
        endDate: this._datePipe.transform(this.editData.endDate, "yyyy-MM-dd"),
        totalSheets: this.editData.totalSheets || "",
        missionThemeId: this.editData.missionThemeId || "",
        missionSkillId: this.editData.missionSkillId?.split(",") || [],
        missionImages: "",
      ...
      this.editMissionForm.patchValue(patchObj);

      if (this.editData.countryId) {
        const citySub = this._commonService.cityList(this.editData.countryId).subscribe((cityData: any) => {
          if (cityData.result === 1) {
            this.cityList = cityData.data;
          }
        })
        this.unsubscribe.push(citySub);
      }

      if (this.editData.missionImages && typeof this.editData.missionImages === "string") {
        this.imageListArray = this.editData.missionImages.split(",").map((img: string) =>
          this._service.imageUrl + "/" + img.replace(/\\/g, "/")
        );
      } else {
        this.imageListArray = [];
      }
  ...
  async onSubmit() {
    ...
    value.missionSkillId = Array.isArray(value.missionSkillId) ? value.missionSkillId.join(",") : ""
    ...
    if (this.editMissionForm.valid) {
      try {
        if (this.isFileUpload) {
          const res: any = await firstValueFrom(this._commonService.uploadImage(this.formData));
          if (res.success) {
            value.missionImages = res.data;
          } else {
            this._toast.error({ detail: "ERROR", summary: "Image upload failed", duration: APP_CONFIG.toastDuration });
            return;
          }
        } else {
          value.missionImages = this.editData.missionImages || "";
...
for (const key in value) {
  if (value.hasOwnProperty(key)) {
    updateForm.append(key, value[key]);
  }
}

const sub = this._service.updateMission(updateForm).subscribe((data: any) => {
          if (data.result == 1) {
            this._toast.success({ detail: "SUCCESS", summary: data.data, duration: APP_CONFIG.toastDuration })
        ...
        this._toast.error({ detail: "ERROR", summary: err.message });
      }
    }
  }
